...
export default function ProcessingCard({ video: initialVideo, onComplete }) {
  ...
  useEffect(() => {
    ...
    const cleanup = onVideoProgress((data) => {
      ...
      setVideo((prev) => ({
        ...
        status: data.stage === 'done'  ? 'completed'
              : data.stage === 'error' ? 'failed'
              : 'processing',
  ...
  return (
    <div className={`bg-gray-900 border rounded-xl p-4 ${
      isFailed ? 'border-red-500/30' : isDone ? 'border-green-500/30' : 'border-blue-500/30'
    }`}>
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`text-xs font-bold ${
          isFailed ? 'text-red-400' : isDone ? 'text-green-400' : 'text-blue-400'
        }`}>
